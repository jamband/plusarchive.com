...
export const Component: React.FC<_Props> = (props) => (
  <button type="button" className={styles.container} onClick={props.sort}>
    {props.children}
    <span
      className={`${styles.iconContainer} ${props.current ? styles.active : styles.inactive}`}
    >
      {props.isAsc && props.type === "string" && (
        <IconArrowDownAZ className={styles.icon} />
      )}
      {props.isAsc && props.type === "number" && (
        <IconArrowDown19 className={styles.icon} />
      )}
      {!props.isAsc && props.type === "string" && (
        <IconArrowDownZA className={styles.icon} />
      )}
      {!props.isAsc && props.type === "number" && (
        <IconArrowDown91 className={styles.icon} />
      )}
    </span>
  </button>
);
